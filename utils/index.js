...
function filterPkgs (willInsPkgs, isCover) {
  ...
  */
  const pkgs = willInsPkgs.filter(p => {
    if(isCover) {
      return true;
    };
    ...
    const pinDependencies = { ...(pinPkg.devDependencies || {}), ...(pinPkg.dependencies || {}) };
...
function installPkgs (pkgs) {
  return new Promise((resolve, reject) => {
    const childProcess = spawn("npm", ["install", ...pkgs, "-D"], { cwd: process.cwd(), stdio: 'inherit' });
